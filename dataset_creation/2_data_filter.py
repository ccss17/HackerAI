...
class CyberDataFilter:
    def __init__(
        ...
        # Allow overriding model via CLI; default remains gemma3:latest
        self.model = model if model else "gemma3:latest"
        # Configurable timeout (seconds) for Ollama requests
        ...
        if self.ollama_available:
            logger.info(
                f"Successfully connected to Ollama with model {self.model}"
            )
        else:
            logger.warning(
                "Could not connect to Ollama. Falling back to rule-based filtering."
            )
    ...
    def check_ollama_availability(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                model_response = requests.get(f"{self.ollama_url}/api/tags")
                logger.info(
                    f"Ollama API tags response: {model_response.status_code}"
                )
    ...
    def query_ollama(
        self, prompt: str, temperature: float = 0.2, timeout: int = 10
    ) -> str:
        ...
        try:
            ...
            logger.error(
                f"Timeout while querying Ollama after {timeout} seconds"
            )
            return ""
        except Exception as e:
...
"""
        response = self.query_ollama(
            prompt, timeout=self.ollama_timeout
        ...
        if response:
            if "YES" in response.upper():
                ...
                is_relevant = False
                reason = (
                    "LLM determined content is not relevant to cybersecurity"
                )
    ...
    def enhance_entry(self, entry: Dict) -> Dict:
        ...
        if len(content) > 1000:  # Further reduced from 2000 to 1000
            logger.info(
                f"Skipping LLM enhancement for large entry ({len(content)} chars)"
            )
            return entry
...
"""
        ...
        # Use configurable timeout per call
        response = self.query_ollama(prompt, timeout=self.ollama_timeout)
        logger.info(
            f"Received response from Ollama: {len(response) if response else 0} chars"
        )
        ...
        try:
            ...
            if (
                "```json" in response
                and "```" in response.split("```json", 1)[1]
            ):
                json_content = (
    ...
    def is_relevant_content(self, text: str) -> bool:
        ...
        high_relevance_matches = sum(
            1
            for kw in self.cybersecurity_keywords["high_relevance"]
            if kw.lower() in text_lower
        )
        medium_relevance_matches = sum(
            1
            for kw in self.cybersecurity_keywords["medium_relevance"]
            if kw.lower() in text_lower
        )

        # Scoring system: high relevance keywords count as 2, medium as 1
        relevance_score = (
            high_relevance_matches * 2
    ...
    def filter_entry(self, entry: Dict) -> Tuple[bool, str]:
        ...
        if self.ollama_available:
            is_relevant, reason = self.assess_cyber_relevance(
                combined_text[:4000]
            )  # Limit length
            return is_relevant, reason
        else:
    ...
    def filter_dataset(
        self, input_file: Path, max_entries: int = None
        ...
        if isinstance(data, dict):
            ...
            if "entries" in data and isinstance(data["entries"], list):
                logger.info(
                    f"Processing {len(data['entries'])} entries from 'entries' key"
                )
                data = data["entries"]
            else:
                data = [data]
        ...
        # Optionally process only a subset; if max_entries is None or 0, process all
        process_count = (
            min(len(data), max_entries)
            if (max_entries and max_entries > 0)
            else len(data)
        )
        logger.info(f"Processing {process_count} entries for this run")
        ...
        logger.info(
            f"Finished processing {process_count} entries: {len(relevant_entries)} relevant, {len(filtered_out_entries)} filtered out"
        )
        return relevant_entries, filtered_out_entries

    def save_filtered_data(
        self, data: List[Dict], original_file: Path, suffix: str = ""
    ) -> bool:
        ...
        try:
            ...
            output_file = (
                self.output_dir
                / f"{original_file.stem}{suffix}_{timestamp}{original_file.suffix}"
            )
    ...
    def process_directory(self, limit=None, max_entries=None):
        """Process files in the input directory. If limit is None or 0, process all files."""
        total_stats = {
            "processed_files": 0,
            "total_entries": 0,
            "retained_entries": 0,
        ...
        if limit == 1:
            ubuntu_files = [
                f for f in files_to_process if "ubuntu_security" in f.name
            ]
            if ubuntu_files:
                files_to_process = ubuntu_files[:1]
                logger.info(
                    "Prioritizing Ubuntu security data file for testing"
                )

        # Limit the number of files if specified (>0)
        ...
        for file_path in files_to_process:
            ...
            logger.info(
                f"Processing {file_path} (file {total_stats['processed_files'] + 1} of {len(files_to_process)})"
            )
            relevant_entries, filtered_entries = self.filter_dataset(
                file_path, max_entries
            )

            if relevant_entries or filtered_entries:
                total_stats["processed_files"] += 1
                total_stats["total_entries"] += len(relevant_entries) + len(
                    filtered_entries
                )
                ...
                if relevant_entries:
                    self.save_filtered_data(
                        relevant_entries, file_path, "_filtered"
                    )

                # Optionally save filtered-out entries for analysis
                if filtered_entries:
                    self.save_filtered_data(
                        filtered_entries, file_path, "_removed"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Filter cybersecurity data using Ollama LLM"
    )
    parser.add_argument(
        "--input-dir", default="raw_data", help="Directory containing raw data"
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Max files to process (default: 0 = all files)",
    )
    parser.add_argument(
        "--max-entries",
        type=int,
        default=0,
        help="Max entries per file to process (default: 0 = all entries)",
    )
    parser.add_argument(
        "--model",
        default=None,
        help="Ollama model name to use, e.g., 'gemma:2b'",
    )
    parser.add_argument(
        "--ollama-timeout",
        type=int,
    ...
    parser.add_argument(
        "--disable-ollama",
        action="store_true",
        help="Disable using Ollama and use only rule-based filtering",
    )
    ...
    # To process ALL files, set --limit 0 (default)
    limit = args.limit if args.limit > 0 else None
    max_entries = args.max_entries if args.max_entries > 0 else None
    filter.process_directory(limit, max_entries)
