#!/usr/bin/env python3
"""
Phi-4 / generic Unsloth SFT fine-tuning (LoRA / QLoRA).
...
- Increase `lora_rank`, `lora_alpha`, and `gradient_accumulation_steps` for deeper tuning;
  adjust `learning_rate` accordingly (e.g., 1e-4 to 2e-4 for Phi-4, lower for larger models).
- The script reads `train/optuna_results/best_hparams.json` by default; override with
...
def setup_wandb(
    ...
    wandb_project: str = "cyber-llm-instruct",
...
def load_model_and_tokenizer(
    ...
    # Try Flash Attention 3 first, fallback to Flash Attention 2 if error
    # Note: Flash Attention 3 requires: pip install flash-attn-3
    attn_implementations = ["flash_attention_3", "flash_attention_2"]
    model = None
    tokenizer = None
    ...
    for attn_impl in attn_implementations:
        try:
            print(f"Attempting to load with {attn_impl}...")
            model, tokenizer = FastLanguageModel.from_pretrained(
                attn_implementation=attn_impl,
                **load_kwargs,
            )
            print(f"✓ Successfully loaded with {attn_impl}")
            used_attn_impl = attn_impl
            break
        except Exception as e:
            print(f"✗ Failed to load with {attn_impl}: {str(e)}")
            if attn_impl == attn_implementations[-1]:  # Last attempt
                print(
                    "All Flash Attention implementations failed, trying without Flash Attention..."
                )
                try:
                    model, tokenizer = FastLanguageModel.from_pretrained(
                        attn_implementation=None,
                        **load_kwargs,
                        # No attn_implementation specified - use default
                    )
                    print("✓ Successfully loaded with default attention")
                    used_attn_impl = "default"
                    break
                except Exception as final_e:
                    print(f"✗ Failed to load model: {str(final_e)}")
                    raise final_e
            continue
    ...
    print(f"Using attention implementation: {used_attn_impl}")

    # Setup chat template for Qwen2.5
    if "qwen" in model_name.lower():
        from unsloth.chat_templates import get_chat_template
        print("Setting up Qwen2.5 chat template...")
...
def format_prompts(
    ...
    for instruction, response in zip(instructions, responses):
        # Use different message format for Qwen2.5 vs other models
        if model_name and "qwen" in model_name.lower():
            # Qwen2.5 uses standard role/content format
            messages = [
                {"role": "user", "content": instruction},
                {"role": "assistant", "content": response},
            ]
        else:
            # Other models use from/value format
            messages = [
...
def main(
    ...
    hparams_path: str = "train/optuna_results/best_hparams.json",
    ...
    Args:
        model_name: Model to fine-tune (e.g., unsloth/Phi-4-mini-instruct, unsloth/Llama-3.3-70B-Instruct)
        resume_from_checkpoint: Path to checkpoint to resume from
        lora_rank: LoRA rank (16 for quick training, 32-64 for better quality)
    ...
    """
    print("=" * 60)
    print("Phi-4 SFT Fine-Tuning Script (Fire CLI)")
    print("=" * 60)
    ...
    # Save LoRA adapters
    model_short = model_name.split("/")[-1].replace("-", "_")
    lora_output_dir = f"outputs/{model_short}_sft_lora"
