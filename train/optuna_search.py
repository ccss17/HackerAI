#!/usr/bin/env python3
"""Optuna hyperparameter search for LoRA-based SFT training.
...
``train/optuna_results/best_hparams.json`` for reuse by ``train/finetune.py``.
"""
...
def format_prompts(
    ...
    """Format instruction/response pairs using the tokenizer chat template."""
...
def sample_hyperparameters(
    ...
    learning_rate = trial.suggest_float(
        "learning_rate", 7e-5, 1.5e-4, log=True
    )
    ...
    lora_rank = 64
    # lora_rank = trial.suggest_categorical("lora_rank", [32, 64])
    # lora_alpha_multiplier = trial.suggest_categorical(
    #     "lora_alpha_multiplier", [1.0, 2.0]
    # )
    ...
    #     "optimizer", ["adamw", "adamw_8bit", "adamw_32bit", "adafactor"]
    # )
    max_grad_norm = 0.3
...
def configure_training_args(
    ...
    per_device_eval_batch_size = min(
        params["per_device_train_batch_size"] * 2, 8
    )
...
def main(
    ...
    use_wandb: bool = True,
    wandb_project: str = "cyber-llm-optuna",
    ...
    """Run Optuna hyperparameter search for SFT fine-tuning."""
    ...
    print(
        "\nNext: run train/finetune.py with the saved hyperparameters for a full training run."
    )
