#!/usr/bin/env python3
"""
Baseline evaluation script for GPT-OSS-120B model using cybersecurity dataset.
...
class GPTOSSBaselineEvaluator:
    """
    Baseline evaluator for GPT-OSS-120B model using cybersecurity dataset.
    ...
    def __init__(self, 
                 model_id: str = "unsloth/gpt-oss-120b-unsloth-bnb-4bit",
                 dataset_path: str = "train/data/test.json",
                 max_seq_length: int = 512,   # Optimized for your dataset
                 use_4bit: bool = True,       # Enable quantization for larger batch size
                 batch_size: int = 32,       # Reduced for 120B model
        ...
        Args:
            ...
            reasoning_effort: GPT-OSS reasoning effort level (low, medium, high)
        """
    ...
    def _load_model(self):
        """Load the GPT-OSS-120B model and tokenizer using Unsloth."""
        print(f"Loading model: {self.model_id}")
        
        # Load model with Unsloth - optimized for GPT-OSS-120B
        self.model, self.tokenizer = FastLanguageModel.from_pretrained(
            model_name=self.model_id,
            max_seq_length=self.max_seq_length,
            dtype=None,  # Auto detection for GPT-OSS
            load_in_4bit=self.use_4bit,
        )
        
        # Speed optimizations for maximum throughput
        ...
        # GPT-OSS uses native chat template - no custom mapping needed
    ...
    def generate_response(self, instruction: str, max_new_tokens: int = 1024, reasoning_effort: str = None) -> str:
        """Generate response for a given instruction."""
        # Use provided reasoning_effort or default from initialization
        ...
        try:
            with torch.no_grad():
                output = self.model.generate(
                    ...
                    num_beams=1,  # Added for GPT-OSS compatibility
    ...
    def generate_batch_responses(self, instructions: List[str], max_new_tokens: int = 1024, reasoning_effort: str = None) -> List[str]:
...
def main(model_id: str = "unsloth/gpt-oss-120b-unsloth-bnb-4bit",
         dataset_path: str = "train/data/test.json",
         num_samples: Optional[int] = None,
         batch_size: int = 32,
         reasoning_effort: str = "medium",
    ...
    Args:
        ...
        batch_size: Batch size for processing (default: 32 for 120B model)
        reasoning_effort: GPT-OSS reasoning effort level (low, medium, high)
    ...
    evaluator = GPTOSSBaselineEvaluator(
        model_id=model_id,
